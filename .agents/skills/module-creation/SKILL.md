...
* **Core wins on collisions.** `I18n::translate()` resolves core (`$translations`) *before* module dictionaries (`$customTranslations`), so a key defined in both silently uses core's value. Two consequences: (a) if you move a string out of core, delete the core copy or your module file is dead weight; (b) the module namespace is flat and shared, so prefix anything ambiguous (`forum_boards`, not `boards`) — two modules defining `category` collide on `scandir` order.
