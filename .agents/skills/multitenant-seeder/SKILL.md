...
2. **Dataset discovery**: scans every `src/Modules/<Module>/Seeders/` folder for a fixed set of recognized filenames — `default.php`, `kitchensink.php` — each returning a plain PHP array (**not JSON**) of `sites`/`users`/`pages`/`media`/etc. rows. Runs them in priority order (`default.php` first, then `kitchensink.php`). Currently only `DemoGenerator` provides these; any module could.
...
- **DemoGenerator's own datasets**: `src/Modules/DemoGenerator/Seeders/{default,kitchensink}.php`, plus its supporting assets under `src/Modules/DemoGenerator/Seeders/data/` (images, videos).
...
- **One-off content-authoring tools** (not part of the `bin/seed` pipeline, run manually): `bin/seed-kitchensink-mass` (bulk-generates stress-test content for the kitchensink demo site).
