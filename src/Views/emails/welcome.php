...
<div style="max-width: 550px; margin: 40px auto; padding: 40px; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; font-family: sans-serif; color: #0f172a;">
    ...
    <p style="font-size: 1rem; line-height: 1.6;">Your administrative account has been successfully initialized on <strong><?= Str::escape($siteName) ?></strong>.</p>
    ...
    <div style="background-color: #0b0f19; border: 1px solid #222636; border-radius: 4px; padding: 20px; color: #00ffcc; font-family: monospace; font-size: 0.9rem; margin-bottom: 30px; word-break: break-all;">
        <strong>Host Domain:</strong> <?= Str::escape($siteDomain) ?><br>
        <strong>Username:</strong> <?= Str::escape($username) ?><br>
        <strong>Email:</strong> <?= Str::escape($email) ?><br>
        <strong>System Role:</strong> <?= Str::escape($role) ?>
    </div>
    <div style="text-align: center; margin-bottom: 35px;">
        <a href="<?= Str::escape($link) ?>" style="display: inline-block; padding: 12px 30px; background-color: #06b6d4; color: #ffffff; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.05em;">Log In to Back-Office</a>
    </div>
